...
def compress_dir_to_nt_gz(directory: Path, artifact_dir: Path):
    """
    Recursively processes RDF files from a directory, combines them into a single graph,
    and writes the output as a gzipped N-Triples file.
    """
    if not directory.is_dir():
        logger.warning(f"Directory not found: {directory}")
        return

    output_file = artifact_dir / f"{directory.name}.nt.gz"
    ...
    for file_path in rdf_files:
        ...
        try:
            ...
            g.parse(location=str(file_path), format=fmt)
        except ParserError as e:
            logger.error(f"Could not parse {file_path}: {e}")
        except Exception as e:
            logger.error(f"An unexpected error occurred with file {file_path}: {e}")

    logger.info(f"Serializing graph for {directory.name} to N-Triples format.")
    serialized_data = g.serialize(format="nt")
...
if __name__ == "__main__":
    ...
    for d in dirs_to_process:
        compress_dir_to_nt_gz(d, artifacts_path)